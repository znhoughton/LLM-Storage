...
```{r}
cosine_data = read_csv('../Data/allenai_OLMo-7B_compositional_cosine_diffs.csv') %>%
...
cosine_data_m4 = cosine_data %>%
  ...
  ungroup() %>%
  mutate(CenteredLogOverallFreq = LogOverallFreq - mean(LogOverallFreq)) %>%
  mutate(scaled_odds = scale(log_odds_cosine)) 

options(contrasts = c("contr.sum","contr.sum"))

m4 = brm(scaled_odds ~ CenteredLogOverallFreq * RelFreq,
         data = cosine_data_m4,
         family = gaussian(),
         warmup = 2000,
         iter = 4000,
...
```{r}
cosine_data = read_csv('../Data/allenai_OLMo-1B_cosine_diffs.csv') %>%
  left_join(all_sentences, by = c('binom' = 'Binomial'))
