...
$$
CosineDiff \sim BinomFreq + (1|Item) 
$$
...
cosine_data = cosine_data %>%
  ...
  rename('cosine_dist' = cosine_diffs)

cosine_data_m2 = cosine_data %>%
  filter(LogBinomFreq > -10) #These items might be driving the effect, let's make sure this isn't the case
...
m1 = brm(cosine_dist ~ LogBinomFreq + (1|Item),
         data = cosine_data,
         family = gaussian(),
         warmup = 10000,
         iter = 20000,
...
m2 = brm(cosine_dist ~ LogBinomFreq + (1|Item),
         data = cosine_data_m2,
         family = gaussian(),
         warmup = 2000,
         iter = 4000,
...
cosine_data_m3 = cosine_data %>%
  ...
  mutate(cosine_diff = cosine_dist - first(cosine_dist)) %>%
  group_by(Item) %>%
...
m3 = brm(cosine_diff ~ LogOverallFreq * RelFreq,
         data = cosine_data_m3,
         family = gaussian(),
         warmup = 2000,
         iter = 4000,
...
post_samples_OverallFreq = sum(post_samples_m3$LogOverallFreq < 0) / length(post_samples_m3$LogOverallFreq)
  
post_samples_RelFreq = sum(post_samples_m3$RelFreq > 0) / length(post_samples_m3$RelFreq)

post_samples_overallfreq_relfreq = sum(post_samples_m3$`LogOverallFreq:RelFreq` < 0) / length(post_samples_m3$`LogOverallFreq:RelFreq`)
...
interact_plot(m3, pred = 'LogOverallFreq', modx = 'RelFreq', plot.points = T)
```
...
cosine_data_m4 = cosine_data %>%
  group_by(Item) %>%
  arrange(desc(binom), .by_group = T) %>%
  mutate(log_odds_cosine = log(cosine_dist/first(cosine_dist))) %>% #larger value means more preferred in alphabetical, smaller means more preferred in nonalphabetical
  group_by(Item) %>%
  mutate(LogOverallFreq = log(OverallFreq+1)) %>%
...
m4 = brm(log_odds_cosine ~ LogOverallFreq * RelFreq,
         data = cosine_data_m4,
         family = gaussian(),
         warmup = 2000,
         iter = 4000,
...
post_samples_OverallFreq = sum(post_samples_m4$LogOverallFreq < 0) / length(post_samples_m4$LogOverallFreq)
  
post_samples_RelFreq = sum(post_samples_m4$RelFreq > 0) / length(post_samples_m4$RelFreq)

post_samples_overallfreq_relfreq = sum(post_samples_m4$`LogOverallFreq:RelFreq` < 0) / length(post_samples_m4$`LogOverallFreq:RelFreq`)

print(post_samples_OverallFreq)
print(post_samples_RelFreq)
print(post_samples_overallfreq_relfreq) #not significant but damn close

interact_plot(m4, pred = 'LogOverallFreq', modx = 'RelFreq', plot.points = T)
interact_plot(m4, pred = 'LogOverallFreq', modx = 'RelFreq')
```

## Plots

```{r}
ggplot(data = cosine_data, aes(x = LogBinomFreq, y = cosine_dist, color = RelFreq)) +
  geom_point() + 
  geom_smooth(method='lm') +
  theme_bw()

ggplot(data = cosine_data_m2, aes(x = LogBinomFreq, y = cosine_dist, color = RelFreq)) +
  geom_point() + 
  geom_smooth(method='lm') +
  theme_bw()
```
...
```{r}
ggplot(data = cosine_data, aes(x = log(OverallFreq), y = cosine_dist, color = RelFreq)) +
  geom_point() + 
  geom_smooth(method='lm') +
  theme_bw()

ggplot(data = cosine_data, aes(x = RelFreq, y = cosine_dist, color = log(OverallFreq))) +
  geom_point() + 
  geom_smooth(method='lm') +
  theme_bw()
