...
There is one more important aspect of the present study that is important to highlight. For each binomial, the holistic embeddings were taken from a single sentence context and that context was the same for the alphabetical and nonalphabetical forms. That is, in order to get the holistic embeddings for *men and women* and *women and men*, they were placed within the same sentence. The importance of this is that if we were taking the mean across sentence contexts, one could argue that the sentence contexts for the more frequent form are more variable (which wouldn't be too surprising). However, if the model is simply memorizing entire sentence contexts, it's unclear how the more frequent ordering would become systematically more different for high-frequency binomials. On the other hand, if the model's embeddings for the frequent ordering represent a more context-independent representation, then that representation would be updated across different contexts and explain why the more frequent form diverges in representation for high-frequency binomials. In other words, the fact that the embeddings for the more frequent form are systematically different from the embeddings for the compositional embeddings (which are unordered) for high-frequency binomials, despite being taken from specific sentence contexts, suggests that the embeddings for the more frequent ordering contain some general context-independent knowledge.
...
The account posited above is also plausible for humans, and has been proposed by prominent linguists before under the label of exemplar theories [e.g., @bybee2003]. Exemplar theories have famously argued that humans are storing context-rich exemplars which people are then able to abstract across. The present study demonstrates that large language models may also be storing context-rich exemplars that they are able to abstract across.
