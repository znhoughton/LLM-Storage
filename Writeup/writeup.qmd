...
In order to examine the semantic compositionality of binomials, we examined the semantic embeddings in five different large language models: the GPT-2 [@radford2019] family, the Llama-2 [@touvron2023] family, and the Olmo [@OLMo2024] family.[^1]
...
Experiment 2 is an exploratory analysis examining how representations for binomials emerge throughout training across different hidden layers.
...
Another possibility is that early in training for high-frequency binomials, the large language model's experience with the individual words may largely overlap with the large language model's experience with the frequent form of the binomial (e.g., the model's experience with contexts containing the binomial *bread and butter* also contribute to the large language model's experience with the individual words). Thus, initially these embeddings may be similar until the large language model experiences enough data to learn different representations. As the model experiences more sentence contexts with the binomial, the representation for the more frequent ordering has more opportunities to diverge from the representation of the individual words. This process explains why the same form can give rise to different representations. For example, @fig_representation_dist demonstrates a hypothetical example of a form with a single representation (left), and a form with multiple representations (right). An analogy for the right example might be a word such as "bank", which can mean a financial institution or a river bank. Storage in this sense can be thought of as an abstraction across a distribution of exemplars.

```{r echo = F}
#| label: fig-representation_dist
#| fig-cap: "A distribution of exemplars with a single representation (left) and a distribution of exemplars with two different representations."
unimodal_dist = tibble(values = rnorm(n = 1000, mean = 5, sd = 2))
bimodal_dist = tibble(values = c(rnorm(n=500, mean = 3, sd = 2), rnorm(n=500, mean = 15, sd = 2)))

unimodal_plot = ggplot(unimodal_dist, aes(x=values)) +
  geom_histogram(binwidth = 0.5, fill = 'blue', color='black', alpha=0.7) +
  labs(x = 'Embeddings', y = 'Count') + 
  theme_bw()

bimodal_plot = ggplot(bimodal_dist, aes(x=values)) +
  geom_histogram(binwidth = 0.5, fill = 'blue', color='black', alpha=0.7) +
  labs(x = 'Embeddings', y = 'Count') + 
  theme_bw()

ggarrange(unimodal_plot, bimodal_plot)
```

There is one more important aspect of the present study that is important to highlight. For each binomial, the holistic embeddings were taken from a single sentence context and that context was the same for the alphabetical and nonalphabetical forms. That is, in order to get the holistic embeddings for *men and women* and *women and men*, they were placed within the same sentence. The importance of this is that if we were taking the mean across sentence contexts, one could argue that the sentence contexts for the more frequent form are more variable (which wouldn't be too surprising). However, if the model is simply memorizing entire sentence contexts (without any abstraction across sentence contexts with overlapping tokens), it's unclear how the more frequent ordering would become systematically more different for high-frequency binomials. This is because even for high-frequency binomials the sentence contexts are likely different (i.e., the binomial isn't recurring in the same sentence context over and over again). On the other hand, if the model's embeddings for the frequent ordering represent a more context-independent representation (i.e., an abstraction over contexts), then that representation would be updated across different contexts. This would explain why the more frequent form diverges in representation for high-frequency binomials. In other words, the fact that the embeddings for the more frequent form are systematically different from the embeddings for the compositional embeddings (which are unordered) for high-frequency binomials, despite being taken from specific sentence contexts, suggests that the embeddings for the more frequent ordering represent some general context-independent knowledge.

<!--# another way to think of this argument: If LLMs are just storing everything without learning to abstract, then the representations of words should be extremely context-dependent. If across sentence contexts there is a systematic pattern (such as in our case, where the embeddings for the more frequent form are systematically less similar to the compositional embeddings), that suggests that the model has learned embeddings that are at least somewhat context-independent. -->

The account posited above is also plausible for humans, and has been proposed by prominent linguists before under the label of exemplar theories [e.g., @bybee2003]. Exemplar theories have famously argued that humans store context-rich exemplars which people then abstract across. The present study demonstrates that large language models may also be storing context-rich exemplars that they are able to abstract across.

Finally, our results can also be considered predictions for how humans may store binomials. Future work would do well to examine whether it is also the case that the semantic representations for the more frequent ordering of high-frequency binomials diverge more from the compositional representations. Our results also make predictions about the timescale of learning: for young children, the pattern of results may actually be the opposite from adults, since at earlier checkpoints in our model the embeddings for the more frequent ordering of high-frequency binomials were more similar to the compositional embeddings.
